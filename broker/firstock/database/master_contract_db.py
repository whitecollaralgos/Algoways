...
def process_firstock_nse_data(output_path):
    """
    Processes the Firstock NSE data (NSE_symbols.csv) to generate AlgoWays symbols.
...
def process_firstock_nfo_data(output_path):
    """
    Processes the Firstock NFO data (NFO_symbols.csv) to generate AlgoWays symbols.
...
def process_firstock_bse_data(output_path):
    """
    Processes the Firstock BSE data (BSE_symbols.csv) to generate AlgoWays symbols.
    ...
    # Apply transformation for AlgoWays symbols (no special logic needed for BSE)
    def get_algoways_symbol(broker_symbol):
        return broker_symbol
...
def process_firstock_bfo_data(output_path):
    """
    Processes the Firstock BFO data (BFO_symbols.csv) to generate AlgoWays symbols.
