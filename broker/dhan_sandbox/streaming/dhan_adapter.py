"""
Fixed Dhan WebSocket adapter for AlgoWays.
...
from .dhan_mapping import get_dhan_exchange, get_algoways_exchange

class DhanWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """
    Dhan-specific implementation of the WebSocket adapter.
    Implements AlgoWays WebSocket proxy interface with proper mode mapping.
    """
    ...
    def __init__(self):
        ...
        # Extended mode mapping to handle all possible AlgoWays modes
        self.mode_map = {
            # Standard AlgoWays modes
    ...
    def subscribe(self, symbol: str, exchange: str, mode: int = 2, depth_level: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            # Map AlgoWays mode to Dhan mode with explicit handling of all possible modes
            if mode == 1:
                # Standard LTP mode
                dhan_mode = DhanWebSocket.MODE_LTP
                self.logger.info(f"Mapped AlgoWays mode {mode} (LTP) to Dhan mode '{dhan_mode}'")
            elif mode == 2:
                # Standard QUOTE mode
                dhan_mode = DhanWebSocket.MODE_QUOTE
                self.logger.info(f"Mapped AlgoWays mode {mode} (QUOTE) to Dhan mode '{dhan_mode}'")
            elif mode == 3:
                # Standard DEPTH mode
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(f"Mapped AlgoWays mode {mode} (DEPTH) to Dhan mode '{dhan_mode}'")
            else:
                # All other modes (4-8) map to FULL/DEPTH
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(f"Mapped AlgoWays mode {mode} (DEPTH/FULL) to Dhan mode '{dhan_mode}'")
            
            # Add symbol to subscription tracking
            with self.lock:
                self.subscribed_symbols[symbol] = {
                    "exchange": exchange,  # Store original AlgoWays exchange
            ...
            # Map AlgoWays exchange to Dhan exchange code
    ...
    def _on_ticks(self, ticks: List[Dict[str, Any]]):
        ...
        try:
            ...
            for tick in ticks:
                ...
                # Convert to AlgoWays format for data field
                ...
                # Normalize tick format to AlgoWays standard
    ...
    def _generate_topic(self, symbol: str, exchange: str, mode_str: str) -> str:
        ...
        Args:
            ...
            exchange: The exchange code in AlgoWays format
    ...
    def _normalize_tick(self, tick: Dict[str, Any]) -> Dict[str, Any]:
        """
        Normalize Dhan tick data to AlgoWays format.
        ...
        try:
            ...
            # Ensure exchange is in AlgoWays format
            exchange = tick.get("exchange")
            if exchange:
                algoways_exchange = get_algoways_exchange(exchange)
            else:
    ...
    def validate_tick_parsing(self, ticker_symbols=None, timeout=30):
        ...
        try:
            ...
            original_callbacks = {}
            for mode in [1, 2, 3, 4, 5]:  # AlgoWays modes
