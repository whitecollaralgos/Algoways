        ...
        for item in scrip_data:
            try:
                ...
                if len(parts) >= 3:
                    ...
                    # Map exchange for AlgoWays format
                    exchange_map = {
                        'NSE': 'NSE_INDEX',
                        'BSE': 'BSE_INDEX'
                    }
                ...
                if group_name == 'Securities':
                    ...
                    if len(parts) >= 4:
                        ...
                        try:
                            ...
                            algoways_symbol = f"{base_symbol}{expiry_formatted}FUT"
                    ...
                    if len(parts) >= 6:
                        ...
                        try:
                            ...
                            algoways_symbol = f"{base_symbol}{expiry_formatted}{strike_str}{option_type}"
                    ...
                    # Get AlgoWays exchange
                    algoways_exchange = exchange_map.get(raw_exchange, raw_exchange)
                    
                    # Check if this is a common index symbol
                    if algoways_exchange == 'BSE_INDEX':
                        # Handle BSE indices
                        if item['symbol'].upper() in COMMON_INDEX_MAP[algoways_exchange]['common']:
                        ...
                        elif item['symbol'].upper() in COMMON_INDEX_MAP[algoways_exchange]['sectoral']:
                            # Use sectoral index format
                            record = {
                                'symbol': item['symbol'].upper(),  # Use uppercase symbol
                                'brsymbol': COMMON_INDEX_MAP[algoways_exchange]['sectoral'][item['symbol'].upper()],  # Use mapped brsymbol
                                'name': item.get('symbol', item['dispName']),  # Use symbol field if available
                                'exchange': algoways_exchange,
                                'brexchange': raw_exchange,
                                'token': str(item['excToken']),
                        ...
                        # Handle NSE indices
                        if item['symbol'].upper() in COMMON_INDEX_MAP[algoways_exchange]:
