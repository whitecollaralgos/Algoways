...
def get_order_book(auth):
    ...
    Returns:
        dict: Order book data in AlgoWays format
    """
    try:
        # Get API key from environment
        api_key = os.getenv('BROKER_API_SECRET')
        ...
        # Transform response data to AlgoWays format
        ...
        if response_data['s'] == 'ok':
            ...
            # Transform each order to AlgoWays format
            transformed_orders = []
            for order in response_data['d']:
                #logger.debug(f"[DEBUG] get_order_book - Processing order: {order}")
                try:
                    # Get AlgoWays symbol using symbol and exchange
                    algoways_symbol = get_oa_symbol(
                        symbol=order['sym']['id'],
                        exchange=order['sym']['exch']
                    )
                    #print(f"[DEBUG] get_order_book - AlgoWays symbol lookup for symbol {order['sym']['sym']}: {algoways_symbol}")
                    
                    transformed_order = {
                        'stat': 'Ok',  # AlgoWays expects 'stat' field
                        'data': {
                            'tradingsymbol': algoways_symbol if algoways_symbol else order['sym']['sym'],  # Fallback to Tradejini symbol if AlgoWays not found
                            'exchange': order['sym']['exch'],
                            'token': order['symId'],
                            'exch': order['sym']['exch'],
                            'quantity': order['qty'],
...
def get_trade_book(auth):
    ...
    Returns:
        dict: Trade book data in AlgoWays format {'data': [...], 'status': 'success'}
    """
    try:
        # Get API key from environment
        api_key = os.getenv('BROKER_API_SECRET')
        ...
        # Transform trades directly to AlgoWays format
        transformed_trades = []
        for trade in trades_data:
            try:
                ...
                # Determine the symbol to use (AlgoWays symbol if available)
                final_symbol = ""  
                if algoways_symbol:
                ...
                transformed_trade = {
                    ...
                    "symbol": final_symbol,  # Using AlgoWays symbol here
...
def get_positions(auth):
    ...
    Returns:
        dict: Positions data in AlgoWays format
    """
    try:
        # Get API key from environment
        api_key = os.getenv('BROKER_API_SECRET')
        ...
        if response_data.get('s') == 'ok':
            ...
            for position in positions:
                try:
                    ...
                    # Get AlgoWays symbol - follow same approach as TradeBook implementation
                    algoways_symbol = None
                    try:
                    ...
                    # Format the position data according to AlgoWays format
                    ...
                    logger.debug(f"Position transformed: {tradingsymbol} → {algoways_symbol}")
            ...
            # Return in AlgoWays format - same pattern as orderbook and tradebook
            return {
                "status": "success",
                "data": positions_list
            }
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    try:
        ...
        # Check if this is already in AlgoWays format
        if positions_response.get('status') == 'success' and 'data' in positions_response:
            logger.info("get_open_position - Processing AlgoWays format positions")
            ...
            for position in positions:
                ...
                logger.info(f"get_open_position - Checking AlgoWays position: {pos_symbol} on {pos_exch}, qty: {pos_qty}")
                
                if pos_exch == exchange and pos_symbol == tradingsymbol and pos_qty != 0:
                    logger.info(f"get_open_position - Found matching AlgoWays position: {pos_symbol} with quantity {pos_qty}")
                    return str(pos_qty)
            
            logger.info(f"get_open_position - No matching AlgoWays position found for {tradingsymbol} on {exchange}")
            return '0'
...
def place_smartorder_api(data, auth):
    ...
    try:
        ...
        try:
            ...
            # Format response to match AlgoWays's expected format
...
def close_all_positions(current_api_key, auth):
    ...
    Returns:
        dict: Response with status and message in AlgoWays format
              {
    ...
    try:
        ...
        # Prepare final response in AlgoWays format
